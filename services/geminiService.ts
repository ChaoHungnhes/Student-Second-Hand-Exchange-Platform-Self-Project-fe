...
export const getAIAssistantResponse = async (userPrompt: string) => {
  try {
    const response = await ai.models.generateContent({
      ...
      contents: userPrompt,
      config: {
        systemInstruction: "Bạn là 'UniTrade Bot', trợ lý AI thân thiện dành cho nền tảng trao đổi đồ cũ sinh viên UniTrade. Hãy giúp người dùng về các quy định nền tảng, cách đăng bài, mẹo giao dịch an toàn và giải đáp các câu hỏi thường gặp (FAQ). Luôn trả lời bằng tiếng Việt, ngắn gọn, súc tích và phù hợp với sinh viên.",
        temperature: 0.7,
      },
    });
    // response.text is a getter property, not a method
    return response.text || "Xin lỗi, tôi không thể xử lý yêu cầu này. Vui lòng thử lại sau.";
  } catch (error) {
...
export const moderateContent = async (title: string, description: string) => {
  try {
    ...
    return { ai_status: "OK", ai_note: "Không thể phân tích nội dung tự động lúc này." };
  }
};
