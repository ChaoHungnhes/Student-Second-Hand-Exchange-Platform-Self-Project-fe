...
import { getAdminProductsAPI, createAdminProductAPI, changeProductStatusAPI, updateAdminProductAPI, deleteAdminProductAPI} from '../../config/api';
...
// ✅ HÀM MỚI: Mở Modal Sửa & Fill dữ liệu cũ vào
  ...
  const handleDeleteProduct = async (id: string) => {
    if (!window.confirm("⚠️ CẢNH BÁO: Bạn có chắc chắn muốn XÓA VĨNH VIỄN sản phẩm này không?\nHành động này không thể hoàn tác.")) {
      return;
    }
  ...
  return (
    <div className="space-y-8 animate-in slide-in-from-right-4 duration-500">
      ...
      {isCreateModalOpen && (
  <div className="fixed inset-0 z-50 flex items-center justify-center p-4"> {/* Thêm p-4 để cách lề màn hình mobile */}
    ...
    <div className="relative w-full max-w-2xl bg-white rounded-2xl shadow-2xl z-10 max-h-[90vh] flex flex-col overflow-hidden">
      ...
      <div className="px-8 py-5 border-b border-gray-100 flex justify-between items-center bg-white shrink-0">
        <h3 className="text-xl font-black text-gray-800">Tạo sản phẩm mới</h3>
      ...
      <form onSubmit={handleCreateSubmit} className="flex flex-col flex-1 overflow-hidden">
        ...
        <div className="p-8 overflow-y-auto flex-1 custom-scrollbar">
          <div className="grid grid-cols-2 gap-4">
            <input placeholder="Seller ID" value={newProductData.sellerId} onChange={e => setNewProductData(prev => ({ ...prev, sellerId: e.target.value }))} className="px-4 py-3 border rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none" />
            <input placeholder="Category ID" value={newProductData.categoryId} onChange={e => setNewProductData(prev => ({ ...prev, categoryId: e.target.value }))} className="px-4 py-3 border rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none" />
            <input placeholder="Title" value={newProductData.title} onChange={e => setNewProductData(prev => ({ ...prev, title: e.target.value }))} className="col-span-2 px-4 py-3 border rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none" />
            <textarea placeholder="Description" rows={3} value={newProductData.description} onChange={e => setNewProductData(prev => ({ ...prev, description: e.target.value }))} className="col-span-2 px-4 py-3 border rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none resize-none" />
            <input placeholder="Price" type="number" value={newProductData.price} onChange={e => setNewProductData(prev => ({ ...prev, price: Number(e.target.value) }))} className="px-4 py-3 border rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none" />
            
            <select aria-label="Trạng thái" value={newProductData.status} onChange={e => setNewProductData(prev => ({ ...prev, status: e.target.value }))} className="px-4 py-3 border rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none bg-white">
              {Object.values(ProductStatus).map(s => <option key={s} value={s}>{s}</option>)}
            </select>
            
            <input placeholder="City" value={newProductData.city} onChange={e => setNewProductData(prev => ({ ...prev, city: e.target.value }))} className="px-4 py-3 border rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none" />
            <input placeholder="District" value={newProductData.district} onChange={e => setNewProductData(prev => ({ ...prev, district: e.target.value }))} className="px-4 py-3 border rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none" />
            <input placeholder="Ward" value={newProductData.ward} onChange={e => setNewProductData(prev => ({ ...prev, ward: e.target.value }))} className="px-4 py-3 border rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none" />
            <input placeholder="Address detail" value={newProductData.addressDetail} onChange={e => setNewProductData(prev => ({ ...prev, addressDetail: e.target.value }))} className="col-span-2 px-4 py-3 border rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none" />
            
            {/* Phần chọn ảnh */}
            <div className="col-span-2">
              <label htmlFor="product-images" className="text-sm font-bold block mb-2 text-gray-700">Images</label>
              <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-gray-300 border-dashed rounded-xl cursor-pointer bg-gray-50 hover:bg-gray-100">
                  ...
                  <input id="product-images" type="file" multiple onChange={handleImageChange} className="hidden" />
              </label>
...
{isEditModalOpen && editingProduct && (
  ...
  <div className="p-8 overflow-y-auto flex-1 custom-scrollbar">
    <div className="space-y-6"> {/* Tăng khoảng cách giữa các nhóm */}
      ...
      <div className="bg-indigo-50 p-5 rounded-xl border border-indigo-100">
          <p className="text-xs font-black text-indigo-800 uppercase mb-3 flex items-center gap-2">
            <i className="fa-solid fa-user-shield"></i> Thông tin quản trị
          </p>
          <div className="grid grid-cols-2 gap-4">
            <div>
                <label className="text-[11px] font-bold text-gray-500 uppercase mb-1 block">Người bán (Seller ID)</label>
                <input 
            ...
            <div>
                  <label className="text-[11px] font-bold text-gray-500 uppercase mb-1 block">Trạng thái (Status)</label>
                  <select 
            ...
            <div>
                  <label className="text-[11px] font-bold text-gray-500 uppercase mb-1 block">AI Status</label>
                  <select 
                    value={editingProduct.aiStatus} 
                    onChange={e => setEditingProduct({...editingProduct, aiStatus: e.target.value})} 
                    className={`w-full px-3 py-2 border border-indigo-200 rounded-lg text-sm font-bold bg-white outline-none ${
                      editingProduct.aiStatus === 'OK' ? 'text-green-600' : 'text-red-600'
                    }`}
                  >
            ...
            <div>
                <label className="text-[11px] font-bold text-gray-500 uppercase mb-1 block">Ghi chú Admin (Lý do)</label>
                <input 
                  placeholder="VD: Vi phạm chính sách..." 
                  value={editingProduct.adminNote || ''} 
                  onChange={e => setEditingProduct({...editingProduct, adminNote: e.target.value})} 
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="col-span-2">
          <label className="text-sm font-bold text-gray-700 mb-1 block">Tiêu đề sản phẩm <span className="text-red-500">*</span></label>
          <input 
        ...
        <div>
          <label className="text-sm font-bold text-gray-700 mb-1 block">Giá bán (VNĐ) <span className="text-red-500">*</span></label>
          <input 
            type="number" 
            placeholder="0" 
        ...
        <div>
          <label className="text-sm font-bold text-gray-700 mb-1 block">Danh mục (Category ID)</label>
          <input 
        ...
        <div className="col-span-2">
          <label className="text-sm font-bold text-gray-700 mb-1 block">Mô tả chi tiết</label>
          <textarea 
      ...
      <div className="border-t border-gray-100 pt-5">
        <h4 className="text-sm font-black text-gray-800 uppercase mb-4 flex items-center gap-2">
          <i className="fa-solid fa-location-dot text-indigo-500"></i> Địa chỉ giao dịch
        </h4>
        <div className="grid grid-cols-2 gap-4 bg-gray-50 p-4 rounded-xl">
            
            {/* Ô 1: Tỉnh/Thành */}
            <div>
              <label className="text-xs font-bold text-gray-500 mb-1 block">Tỉnh / Thành phố</label>
              <input 
            ...
            {/* Ô 2: Quận/Huyện */}
            <div>
              <label className="text-xs font-bold text-gray-500 mb-1 block">Quận / Huyện</label>
              <input 
                placeholder="VD: Đống Đa" 
                value={editingProduct.district || ''} 
                onChange={e => setEditingProduct({...editingProduct, district: e.target.value})} 
                className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:border-indigo-500 outline-none bg-white" 
              />
            </div>
            ...
            {/* Ô 3: Phường/Xã */}
            <div>
              <label className="text-xs font-bold text-gray-500 mb-1 block">Phường / Xã</label>
              <input 
                placeholder="VD: Láng Hạ" 
                value={editingProduct.ward || ''} 
                onChange={e => setEditingProduct({...editingProduct, ward: e.target.value})} 
                className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:border-indigo-500 outline-none bg-white" 
              />
            </div>
            ...
            {/* Ô 4: Chi tiết */}
            <div>
              <label className="text-xs font-bold text-gray-500 mb-1 block">Số nhà / Tên đường</label>
              <input 
                placeholder="VD: Số 15 ngõ 99" 
                value={editingProduct.addressDetail || ''} 
                onChange={e => setEditingProduct({...editingProduct, addressDetail: e.target.value})} 
                className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:border-indigo-500 outline-none bg-white" 
              />
            </div>
        </div>
  ...
  <div className="px-8 py-5 border-t border-gray-100 bg-gray-50 flex justify-end gap-3 shrink-0">
    <button type="button" onClick={() => setIsEditModalOpen(false)} className="px-6 py-2.5 rounded-xl border border-gray-300 font-bold text-gray-700 hover:bg-gray-100">Hủy</button>
    <button type="submit" className="px-6 py-2.5 rounded-xl bg-indigo-600 font-bold text-white hover:bg-indigo-700 shadow-lg shadow-indigo-200 active:scale-95 transition-all">Lưu thay đổi</button>
  </div>
</form>
          </div>
        </div>
                  ...
                  <td className="px-8 py-5">
                    <div className="flex items-center gap-4">
                      {p.imageUrls && p.imageUrls[0] && (
                        <img src={getImageUrl(p.imageUrls[0])} className="w-12 h-12 rounded-2xl object-cover" alt="product" />
                      )}
