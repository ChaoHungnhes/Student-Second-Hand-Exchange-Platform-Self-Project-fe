...
const findFallbackProducts = async (message: string): Promise<ProductSuggestion[]> => {
    ...
    try {
      ...
      if (products.length > 0) {
        return products.map((product: any) => ({
          ...
          image: product.image || product.imageUrl || product.imageUrls?.[0] || '',
          city: product.city,
          ward: product.ward,
        }));
      }
...
const ProductSuggestionCard: React.FC<{ product: ProductSuggestion }> = ({ product }) => (
  <div className="flex gap-3 rounded-2xl border border-emerald-100 bg-white p-2 shadow-sm">
    ...
    <div className="min-w-0 flex-1">
      <h4 className="line-clamp-2 text-sm font-bold text-slate-900" title={product.title}>{product.title}</h4>
      <p className="mt-1 text-sm font-extrabold text-emerald-600">{currencyFormatter.format(product.price || 0)}</p>
      <p className="mt-1 text-xs text-slate-500"><i className="fa-solid fa-layer-group mr-1"></i>{product.categoryName || 'Khác'}</p>
      <p className="text-xs text-slate-500 truncate"><i className="fa-solid fa-location-dot mr-1 text-emerald-500"></i>{[product.ward, product.city].filter(Boolean).join(', ')}</p>
      <Link
...
const ChatBotWidget: React.FC = () => {
  ...
  const sendMessage = async (rawMessage?: string) => {
    ...
    const userMessage: ChatMessage = { id: createMessageId(), role: 'user', text, createdAt: new Date() };
    setMessages((current) => [...current, userMessage]);
    setInput('');
    setIsSending(true);

    try {
      const response: any = await chatBotAPI(text);
      const responseProducts = Array.isArray(response?.products) ? response.products : [];
      const fallbackProducts = responseProducts.length > 0 ? [] : await findFallbackProducts(text);
      const products = responseProducts.length > 0 ? responseProducts : fallbackProducts;
      const botMessage: ChatMessage = {
        id: createMessageId(),
        role: 'bot',
        text: products.length > 0
          ? response?.answer || 'Mình tìm thấy một số sản phẩm phù hợp trên S2S:'
          : response?.answer || 'Mình chưa tìm thấy câu trả lời phù hợp. Bạn thử hỏi cách khác nhé.',
        products,
        createdAt: new Date(),
      };
    ...
    <div className="fixed bottom-4 right-4 z-50 sm:bottom-6 sm:right-6">
      ...
      {isOpen && (
        <section className="flex h-[calc(100vh-32px)] w-[calc(100vw-32px)] max-w-[400px] flex-col overflow-hidden rounded-[28px] border border-white/70 bg-white shadow-2xl shadow-slate-900/20 animate-[s2s-pop_.18s_ease-out] sm:h-[600px]">
          ...
          <header className="relative overflow-hidden bg-gradient-to-r from-emerald-600 via-teal-600 to-cyan-600 p-4 text-white">
            ...
            <div className="relative flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-white/20 backdrop-blur"><i className="fa-solid fa-robot"></i></div>
                <div>
                  <h3 className="text-base font-extrabold">Trợ lý S2S</h3>
                  <p className="text-xs text-emerald-50">Gợi ý đồ cũ sinh viên trong vài giây</p>
                </div>
              </div>
              <button type="button" onClick={() => setIsOpen(false)} className="rounded-full p-2 text-white/90 transition hover:bg-white/15" aria-label="Thu nhỏ chatbot">
          ...
          <div className="flex-1 space-y-4 overflow-y-auto bg-[radial-gradient(circle_at_top_left,#ecfeff,transparent_35%),linear-gradient(180deg,#f8fafc,#eefdf7)] p-4">
            {messages.length === 0 && (
              <div className="rounded-3xl border border-emerald-100 bg-white/85 p-4 shadow-sm">
                <p className="text-sm font-semibold text-slate-800">Bạn cần tìm gì trên S2S hôm nay?</p>
                <div className="mt-3 flex flex-wrap gap-2">
                  {suggestedQuestions.map((question) => (
                    <button key={question} type="button" onClick={() => sendMessage(question)} className="rounded-full border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs font-semibold text-emerald-700 transition hover:bg-emerald-100">
                      {question}
                    </button>
                  ))}
                </div>
            ...
            {messages.map((message) => (
              <div key={message.id} className={`flex ${message.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[86%] rounded-3xl px-4 py-3 text-sm shadow-sm ${message.role === 'user' ? 'rounded-br-md bg-slate-900 text-white' : 'rounded-bl-md border border-emerald-100 bg-white text-slate-700'}`}>
                  <p className="whitespace-pre-wrap leading-relaxed">{message.text}</p>
                  {message.products && message.products.length > 0 && (
                    <div className="mt-3 space-y-2">
                      {message.products.map((product) => <ProductSuggestionCard key={product.id} product={product} />)}
                    </div>
                  )}
                </div>
              </div>
            ...
            {isSending && (
              <div className="flex justify-start">
                <div className="rounded-3xl rounded-bl-md border border-emerald-100 bg-white px-4 py-3 text-sm font-medium text-slate-500 shadow-sm">
                  <span className="mr-2 inline-flex h-2 w-2 animate-pulse rounded-full bg-emerald-500"></span>S2S đang trả lời...
                </div>
              </div>
            )}
          ...
          <form className="border-t border-slate-100 bg-white p-3" onSubmit={(event) => { event.preventDefault(); sendMessage(); }}>
            <div className="flex items-center gap-2 rounded-2xl border border-slate-200 bg-slate-50 p-2 focus-within:border-emerald-400 focus-within:bg-white">
              ...
              />
              <button type="submit" disabled={isSending || !input.trim()} className="flex h-10 w-10 items-center justify-center rounded-xl bg-emerald-600 text-white transition hover:bg-emerald-700 disabled:cursor-not-allowed disabled:bg-slate-300" aria-label="Gửi tin nhắn">
