    ...
    <div className="max-w-7xl mx-auto py-8 space-y-8 px-4">
      ...
      <div className="bg-white rounded-3xl p-6 border border-gray-100 shadow-sm flex flex-col md:flex-row justify-between items-center gap-6">
        <div>
          ...
          <p className="text-gray-500 text-sm">Quản lý tin đăng bán và lịch sử mua sắm của bạn.</p>
        </div>
        <div className="flex bg-gray-100 p-1 rounded-2xl">
          <button 
            onClick={() => { setActiveTab('LISTINGS'); setFilterStatus('ALL'); setSearchTerm(''); }}
          ...
          <button 
            onClick={() => { setActiveTab('PURCHASED'); setSearchTerm(''); }}
          ...
          {[
            ...
            <button
              ...
              className={`px-4 py-2 rounded-xl text-sm font-bold whitespace-nowrap transition-all ${
                filterStatus === tab.id ? 'bg-indigo-600 text-white shadow-md' : 'bg-white text-gray-500 border border-gray-100 hover:bg-gray-50'
              }`}
            >
              {tab.label}
            </button>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {filteredList.map((p, index) => (
            <div key={`${p.id}-${index}`} className="relative group">
              ...
              {activeTab === 'LISTINGS' && (p.status === 'PENDING' || p.status === 'REJECTED' || p.aiStatus === 'SCAM') && (
                <div className="absolute top-2 left-2 z-10 flex flex-col gap-1">
                  {(p.aiStatus === 'WARNING' || p.aiStatus === 'SCAM') && (
                    <span className={`px-2 py-1 rounded-md text-[9px] font-black uppercase flex items-center gap-1 text-white ${
