...
const ConversationPage: React.FC = () => {
        ...
        try {
          ...
          if (productData) {
            ...
            if (String(sellerId) === String(user.id)) {
              console.log(
                "✅ [DEBUG] MATCH! User là người bán -> setIsOwner(true)",
              );
            ...
            if (
              ...
              try {
                console.log(
                  "🔍 Đang tìm Transaction ID cho sản phẩm:",
                  currentDetails.productId,
                );
                ...
                if (txData && txData.id) {
                  ...
                  console.warn(
                    "⚠️ Không tìm thấy ID trong response transaction",
                  );
                }
              } catch (txError) {
  ...
  const connectWS = () => {
    const socket = new SockJS("http://localhost:8089/ws", null, {
